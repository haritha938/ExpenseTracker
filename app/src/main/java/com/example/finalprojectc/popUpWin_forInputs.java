...
public class popUpWin_forInputs extends DialogFragment {
    ...
    public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
        ...
        editTextAnnualIncome.setOnFocusChangeListener(new View.OnFocusChangeListener() {
            @Override
            public void onFocusChange(View v, boolean hasFocus) {
                if(!hasFocus)
                {
                    if(editTextAnnualIncome.getText().equals(""))
                    {
                        editTextAnnualIncome.setError("Can't be Empty");
                    }
                }
            }
        });
        ...
        editTextAnnualSavings.setOnFocusChangeListener(new View.OnFocusChangeListener() {
            ...
            public void onFocusChange(View v, boolean hasFocus) {
                if(!hasFocus)
                {
                    if(editTextAnnualSavings.getText().equals(""))
                    {
                        editTextAnnualSavings.setError("Can't be Empty");
                    }
                    else if( editTextAnnualIncome.getText().length()>0 && editTextMaxDailyExpense.getText().length()>0 && editTextAnnualSavings.getText().length()>0 ) {
                        ...
                        {
                            Toast.makeText(getContext(),"Max Daily Expense is updated!.",Toast.LENGTH_SHORT).show();
        ...
        editTextMaxDailyExpense.setOnFocusChangeListener(new View.OnFocusChangeListener() {
            ...
            public void onFocusChange(View v, boolean hasFocus) {
                if(!hasFocus )
                {
                    if(editTextMaxDailyExpense.getText().equals(""))
                    {
                        editTextMaxDailyExpense.setError("Can't be Empty");
                    }
                    else if(editTextAnnualIncome.getText().length()>0 && editTextAnnualSavings.getText().length()>0 &&editTextMaxDailyExpense.getText().length()>0) {
                        AnnualIncome = Double.parseDouble(editTextAnnualIncome.getText().toString());
                        DesiredAnnualSavings = Double.parseDouble(editTextAnnualSavings.getText().toString());
                        MaxDailyExpenseLimit = Double.parseDouble(editTextMaxDailyExpense.getText().toString());
                        if (Double.parseDouble(editTextMaxDailyExpense.getText().toString()) > Math.round((Double.parseDouble(editTextAnnualIncome.getText().toString()) - Double.parseDouble(editTextAnnualSavings.getText().toString())) / 365)) {
                            ...
                            editTextMaxDailyExpense.setError(null);
                            editTextAnnualSavings.setError(null);
                        }
                    }
            ...
            builder .setPositiveButton("Save", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int id) {
                        if(editTextAnnualIncome.getText().equals("")||editTextAnnualSavings.getText().equals("")||editTextMaxDailyExpense.getText().equals(""))
                        {

                            Toast.makeText(getContext(), "No input field should be empty ", Toast.LENGTH_LONG).show();
                        ...
                        else
                        {
                            ...
                            if (MaxDailyExpenseLimit == Math.round((AnnualIncome - DesiredAnnualSavings) / 365)) {
                                if (databaseHelper.areGoalsSet(userId)>0)
                                {
